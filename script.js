document.addEventListener('DOMContentLoaded', () => {
    // --- FIX FOR ERROR ---
    // When loading Supabase via CDN, it attaches to the global window object.
    // We must destructure 'createClient' from 'window.supabase' before using it.
    const { createClient } = window.supabase;
  
    // Initialize Supabase
    // IMPORTANT: Replace with your actual Supabase URL and Anon Key
    const SUPABASE_URL = 'https://qgayglybnnrhobcvftrs.supabase.co';
    const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFnYXlnbHlibm5yaG9iY3ZmdHJzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjI2ODQ5ODMsImV4cCI6MjA3ODI2MDk4M30.dqiEe-v1cro5N4tuawu7Y1x5klSyjINsLHd9-V40QjQ';
        ...
        if (navigator.onLine) {
            try {
                if (id && !id.startsWith('offline_')) {
                    ...
                    if (table === 'inventory' && result && result.length > 0) {
                        const newId = result[0].id;
                        const index = inventory[data.section].findIndex(item => item.id === id);
                        if (index !== -1) {
                            inventory[data.section][index].id = newId;
    ...
    async function addNewItem() {
        ...
        const newItem = { 
            ...
            createdBy: currentUser ? currentUser.id : 'offline_user',
    ...
    function editInventoryItem(section, itemId) {
        ...
        if (item) {
            ...
            document.getElementById('editInventoryExpiry').value = item.expiryDate || '';
    ...
    async function updateInventoryItem() {
        ...
        if (item) {
            const updatedItem = {
                ...
                description,
                status: stock > 10 ? 'in-stock' : (stock > 0 ? 'low-stock' : 'out-of-stock'),
                updatedBy: currentUser ? currentUser.id : 'offline_user',
                updatedAt: new Date().toISOString()
            };
            
            await saveDataToSupabase('inventory', updatedItem, itemId);
    ...
    async function completeCheckout() {
        ...
        const saleRecord = {
            ...
            userEmail: currentUser ? currentUser.email : 'offline@example.com', 
            section, 
            items: saleItems, 
            subtotal, 
            total: subtotal,
        ...
        await saveDataToSupabase('sales', saleRecord);
        ...
        // Save updated stats
        await saveDataToSupabase('salesData', salesData[section], section);
        await saveDataToSupabase('userData', userData[section], section);
    ...
    async function saveAccountInfo(section) {
        ...
        if (currentUser && navigator.onLine) {
            ...
            updateUserInfo({ ...currentUser, user_metadata: { full_name: fullName } });
            const userDataRecord = { uid: currentUser.id, displayName: fullName, email, phone, section };
            await saveDataToSupabase('users', userDataRecord, currentUser.id);
            showNotification('Account information saved successfully', 'success');
        } else {
            // Save locally for offline use
            const userDataRecord = { displayName: fullName, email, phone, section };
            saveToLocalStorage(`userProfile_${section}`, userDataRecord);
